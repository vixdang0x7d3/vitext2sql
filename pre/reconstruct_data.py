    ...
    for i in range(len(ddl_file)):
        if remove_digits(table_names[i]) in representatives:
            if len(representatives[remove_digits(table_names[i])]) > 10:
                if (
                    ddl_file["table_name"][i]
                    != representatives[remove_digits(table_names[i])][0]
                ):
    ...
    if dialect == "sqlite":
        ...
        for i in range(len(table_name_list)):
            if os.path.exists(os.path.join(db_name_path, table_name_list[i] + ".json")):
                with open(
                    os.path.join(db_name_path, table_name_list[i] + ".json"),
                    encoding="utf-8",
                ) as f:
            ...
            for j in range(len(table_json[f"{column_prefix}names"])):
                ...
                if add_description:
                    if j < len(table_json["description"]):
                        table_des = (
                            " Description: " + str(table_json["description"][j])
                            if table_json["description"][j]
                            else ""
                        )
                ...
                if reduce_col and ddl_sl_flag:
                    if table_json[f"{column_prefix}names"][j] in col_names:
                        prompts += (
                            "Column name: "
                            + table_json[f"{column_prefix}names"][j]
                            + " Type: "
                            + table_json[f"{column_prefix}types"][j]
                            + table_des
                            + "\n"
                        )
                else:
            ...
            if add_sample_rows:
                if reduce_col and ddl_sl_flag:
                    sample_rows = [
                        {
                            col: row[col]
                            for col in extract_column_names(col_names)
                            if col in row
                        }
            ...
            if representatives is not None:
                if remove_digits(table_name_list[i]) in representatives:
                    if len(representatives[remove_digits(table_name_list[i])]) > 1:
                        assert (
                            len(representatives[remove_digits(table_name_list[i])])
                            >= 10
                        ), representatives[remove_digits(table_name_list[i])]
