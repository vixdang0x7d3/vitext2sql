...
with st.sidebar:
    st.header("Control")
    db_name = st.selectbox("DB Name", ("perpetrator", "baseball_1"))
    db_des = True
...
if prompt:
    ...
    results = retrieve_from_collections(prompt, db_name)
    save_prompt_context(results=results, id=id, db_name=db_name)
    ...
    with st.chat_message("assistant"):
        ...
        st.dataframe(
            assistant_response_content["results_data"],
            use_container_width=True,
            hide_index=True,
        )
